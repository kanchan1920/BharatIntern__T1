...
app.post('/add-blog', async(req, res) => {
    ...
    const { fname, lname, email, title, image1,image2,image3, content } = req.body;

    const isdatastored=await Blog.create({ name: fname + " " + lname, email: email, photo: [image1,image2,image3], text: content, title: title });
    if (!isdatastored) {
        console.log('Err');
    }
